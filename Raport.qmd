...
```{r}
avg_possession <- dane %>% group_by(Squad) %>% summarise(AvgPossession = median(Possession), NumSeasons = n_distinct(Season))
...
ggplot(avg_possession, aes(x = reorder(Squad, -AvgPossession), y = AvgPossession, fill = Color)) +
  ...
  labs(title = "Średnia Posiadania Piłki na Przez Wszystkie Sezony",
       x = "Drużyna",
       y = "Średnia Posiadania (%)")+
       geom_hline(yintercept = mean(avg_possession$AvgPossession), linetype="dashed", color = "black", size = 1)+
...
p <- ggplot(selected_teams, aes(x = Squad))+
  geom_bar(stat = "identity", position = "dodge", aes(y=AvgGoals, fill="Goals"))+
  geom_bar(stat = "identity", position = "dodge", aes(y=AvgEg, fill="Expected goals", width = 0.2))
ggplotly(p)
```
...
p <- ggplot(selected_teams, aes(x = Squad))+
  ...
  geom_bar(stat = "identity", aes(y=AvgEg, fill="Expected goals"))
ggplotly(p)
```
