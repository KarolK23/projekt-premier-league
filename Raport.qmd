...
leaflet() %>%
  ...
  addMarkers(lng=-4.2912131, lat=53.4630716, popup="King Power Stadium")  
```
...
```{r}
summary(mod_p)
pod_mod <- podania[,-c(1,2)]
mod_p <- lm(Assists~., pod_mod)
summary(mod_p)
olsrr::ols_step_forward_aic(mod_p)
model <- lm(Assists~Passes.into.penalty.area+Through.balls+Passes.completed, pod_mod)
summary(model)
AIC(model)
AIC(mod_p)
```
...
radar <- podania %>% group_by(Squad) %>% 
  summarise(proc.short=(sum(Short.passes.attempted)/sum(Passes.attempted))*100,
            proc.mid=(sum(Medium.passes.attempted)/sum(Passes.attempted))*100,
            proc.long=(sum(Long.passes.attempted)/sum(Passes.attempted))*100,
            proc.cross=(sum(Crosses)/sum(Passes.attempted))*100,
            proc.switches=(sum(Switches/sum(Passes.attempted))*100))
mfrow=c(1,2)
x <- radar[c(1,7,12),-1]
x <- rbind(rep(45,5) , rep(0,5) , x)
colors_border=c( rgb(0.2,0.5,0.5,0.9), rgb(0.8,0.2,0.5,0.9) , rgb(0.7,0.5,0.1,0.9) )
colors_in=c( rgb(0.2,0.5,0.5,0.4), rgb(0.8,0.2,0.5,0.4) , rgb(0.7,0.5,0.1,0.4) )
radarchart( x  , axistype=2 , 
...
y <- radar[7,-1]
y <- rbind(rep(45,5) , rep(0,5) , y)
radarchart( y  , axistype=2 , 
...
x <- data_for_clustering %>% mutate(short=round(mean(proc.short),2), mid=round(mean(proc.mid),2), long=round(mean(proc.long),2))

chuj <- x[4:6] %>% group_by(short, mid, long) %>% summarise()
...
data <- rbind(c(5, 50, 20, 50, 50, 50, 50, 50, 50, 50) , rep(0,10) , data)
...
```{r}
kox <- podania %>% group_by(Squad) %>% 
  summarise(`Podania celne`=sum(Passes.completed),
            `Krótkie`=sum(Short.passes.completed),
            `Średnie`=sum(Medium.passes.completed),
            `Długie`=sum(Long.passes.completed),
            `Progresywne`=sum(Progressive.passes),
            `Dośrodkowania`=sum(Crosses),
            `Na dobieg`=sum(Through.balls),
            `W pole karne`=sum(Passes.into.penalty.area))

kox <- kox[c(1,8,15,16,17,25),-1]
data <- rbind(c(max(kox[,1]), max(kox[,2]), max(kox[,3]), max(kox[,4]), max(kox[,5]), max(kox[,6]), max(kox[,7]), max(kox[,8]))*1.1 , c(min(kox[,1]), min(kox[,2]), min(kox[,3]), min(kox[,4]), min(kox[,5]), min(kox[,6]), min(kox[,7]), min(kox[,8]))*0.1 , kox)
...
```{r}
kox <- dane %>% group_by(Squad) %>% 
  summarise(`Bramki`=sum(Tackles.att.3rd)/sum(Tackles),
            `Akcje bramkowe`=sum(Goal.creatiing.actions),
            `Bramki z gry`=sum(GCA.Live.ball.pass),
            `Bramki po stałym fragmencie`=sum(GCA.Dead.ball.pass),
            `Bramki po minieciu przeciwnika`=sum(GCA.Take.on),
            `Bramki po strzale`=sum(GCA.Shot),
            `Bramki po faulu`=sum(GCA.Fouls.drawn),
            `Bramki po akcji z obrony`=sum(GCA.Deffensive.action))
